...
namespace BullBeez.WebAdmin.Controllers
{
    public class CompanyTypeController : Controller
    {
        ...
        public async Task<ActionResult> GetCompanyTypeList(DataTablesRequest dataRequest)
        {
            var json = _Serializer.Serialize(new { take = dataRequest.length, skip = dataRequest.start, search = dataRequest.Search.Value, CommonType = 4 });
        ...
        {
            var json = _Serializer.Serialize(new { Id = request.Id, CommonType = 4 });
        ...
        public async Task<ActionResult> GetCompanyTypeInsetOrUpdate(CommonDataInsertOrUpdate request)
        {
            request.CommonType = 4;
